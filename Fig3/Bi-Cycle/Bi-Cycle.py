...
for center_diagonal_value in range(6, 16):
    ...
    print(
        f'开始生成数据：'
        f'C[center, center] = {center_diagonal_value}'
    )
    ...
    while len(data) < 1000:
        ...
        for j in range(G):
            next_node = (j + 1) % G
            C[next_node, j] = np.random.randint(1, 4)
            C[j, next_node] = np.random.randint(1, 4)
    ...
    # 当前 center_diagonal_value 已生成 1000 条数据
    df = pd.DataFrame(
        data,
        columns=columns
    )
    ...
    file_name = (
        f'Bi-Cycle_{center_diagonal_value}.csv'
    )
