...
for outer_center_value in range(6, 16):

    # 每个参数值单独保存 1000 条有效数据
    data = []
    attempt_count = 0
    ...
    while len(data) < 1000:
        ...
        outer_center = np.random.choice(
            outer_nodes
        )

        # 外围大 c 节点的主对角线元素依次固定为 6～15
        C[outer_center, outer_center] = outer_center_value

        # 双向 Star：中心节点与所有外围节点双向连接
        for j in range(G):
            if j != center:
    ...
    summary_data.append({
        ...
        'alpha*=structure Yes count': (
            structure_yes_count
        ),
        'alpha*=structure Yes ratio': (
            structure_yes_ratio
        ),
        'alpha*=center Yes count': (
            center_yes_count
        ),
        'alpha*=center Yes ratio': (
            center_yes_ratio
        )
    })
