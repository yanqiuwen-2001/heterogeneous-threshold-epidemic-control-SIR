...
# 1. 全局配置
# ============================================================
SEEDS = [5,21,66,87,42,12,2026,37,101,3407]
...
def predict_pairwise_right_probs(model, scaler, X_pair: np.ndarray, batch_size: int, device, seed: int, right_choice_index: int = 1):
...
def train_one_split(
    ...
    for epoch in range(max_epochs):
        ...
        print(
            f"[seed={seed}] Epoch [{epoch+1:03d}/{max_epochs}] | "
...
def evaluate_on_holdout(model, scaler, X_test, y_test, batch_size, device, seed, threshold=0.5, positive_class_index: int = 1):
...
def evaluate_multiclass_only(
    ...
    if true_col is None:
        print("Warning: 未识别到 8 分类真实标签列，只返回空指标。")
        return None
    ...
    try:
        ...
        print(f"Warning: 8分类 macro-AUC 计算失败：{e}")
        macro_auc_8 = np.nan
    ...
    return {
        ...
        "macro_auc": float(macro_auc_8) if not np.isnan(macro_auc_8) else np.nan,
...
def run_one_seed(
    ...
    # D. 在验证集上选阈值
    best_threshold, _ = choose_threshold_by_youden_j(
        best_model,
        result["val_loader"],
        DEVICE,
    ...
    binary_auc = np.nan if np.isnan(test_result["auc"]) else float(test_result["auc"])

    # F. 用训练好的二分类模型对 8 分类数据做预测并评估
    ...
    if os.path.exists(MULTI8_DATA_PATH):
        ...
        multiclass_metrics = evaluate_multiclass_only(
            real_data=real_data_8,
        ...
        print(f"Warning: 未找到 8 分类数据文件 {MULTI8_DATA_PATH}，跳过 8 分类预测与评估。")

    row = {
        "seed": int(seed),
        "binary_accuracy": float(test_result["accuracy"]),
        "binary_auc": binary_auc,
        "binary_f1": float(f1_arr[1]),
        "binary_precision": float(precision_arr[1]),
        "binary_recall": float(recall_arr[1]),
        "binary_best_epoch": int(result["best_epoch"]),
        "binary_best_val_loss": float(result["best_val_loss"]),
        "binary_best_val_acc": float(result["best_val_acc"]),
        "binary_threshold": float(best_threshold),
        "multiclass_accuracy": np.nan if multiclass_metrics is None else multiclass_metrics["accuracy"],
        "multiclass_macro_auc": np.nan if multiclass_metrics is None else multiclass_metrics["macro_auc"],
        "multiclass_macro_f1": np.nan if multiclass_metrics is None else multiclass_metrics["macro_f1"],
    }

    binary_auc_str = "nan" if pd.isna(row["binary_auc"]) else f"{row['binary_auc']:.4f}"
    print(f"[seed={seed}] 二分类: acc={row['binary_accuracy']:.4f}, auc={binary_auc_str}, f1={row['binary_f1']:.4f}")
    if multiclass_metrics is not None:
        mc_auc_str = "nan" if pd.isna(row["multiclass_macro_auc"]) else f"{row['multiclass_macro_auc']:.4f}"
        print(f"[seed={seed}] 八分类: acc={row['multiclass_accuracy']:.4f}, macro_auc={mc_auc_str}, macro_f1={row['multiclass_macro_f1']:.4f}")

    return row


# ============================================================
# 9. 主流程：多随机种子循环，并把所有结果输出到一个文件里
# ============================================================
def main():
    _, X_all, y_all, label_encoder = load_binary_data(BINARY_DATA_PATH)
    ...
    numeric_cols = [
        "binary_accuracy", "binary_auc", "binary_f1", "binary_precision", "binary_recall",
        "binary_best_epoch", "binary_best_val_loss", "binary_best_val_acc", "binary_threshold",
        "multiclass_accuracy", "multiclass_macro_auc", "multiclass_macro_f1"
    ]

    mean_row = {"seed": "mean"}
    std_row = {"seed": "std"}
    for col in numeric_cols:
        mean_row[col] = summary_df[col].astype(float).mean(skipna=True)
        std_row[col] = summary_df[col].astype(float).std(skipna=True, ddof=1)

    summary_out_df = pd.concat([
        summary_df,
        pd.DataFrame([mean_row, std_row])
    ], ignore_index=True)

    summary_csv_path = os.path.join(SAVE_DIR, "all_seed_results_summary.csv")
    summary_out_df.to_csv(summary_csv_path, index=False, encoding="utf-8-sig")

    summary_json_path = os.path.join(SAVE_DIR, "all_seed_results_summary.json")
    with open(summary_json_path, "w", encoding="utf-8") as f:
        json.dump({
            "seeds": SEEDS,
            "binary_data_path": BINARY_DATA_PATH,
            "multiclass_data_path": MULTI8_DATA_PATH,
    ...
    print("\n" + "=" * 80)
    print("全部随机种子运行完成。")
    print(f"所有 seed 的结果已汇总到一个 CSV 文件: {summary_csv_path}")
    print(f"同时保存了一个 JSON 文件: {summary_json_path}")
    print("=" * 80)
