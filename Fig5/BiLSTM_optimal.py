...
def save_eps_and_png(fig, eps_path: str, dpi: int = 300):
    eps_path = os.fspath(eps_path)
...
# 7. 绘图辅助函数（二分类）
# ============================================================
# def plot_loss_curve(ax, history_df: pd.DataFrame):
#     ax.plot(history_df["epoch"], history_df["train_loss"], color="red", label="Train Loss")
#     ax.plot(history_df["epoch"], history_df["val_loss"], color="blue", label="Val Loss")
#     ax.set_xlabel("Epoch")
#     ax.set_ylabel("Loss")
...
#
#
# def plot_accuracy_curve(ax, history_df: pd.DataFrame):
#     ax.plot(history_df["epoch"], history_df["train_acc"], color="red", label="Train Accuracy")
#     ax.plot(history_df["epoch"], history_df["val_acc"], color="blue", label="Val Accuracy")
#     ax.set_xlabel("Epoch")
...
#
#
# def plot_binary_confusion_matrix(ax, y_true, y_pred, target_names):
#     cm = confusion_matrix(y_true, y_pred, labels=np.arange(len(target_names)))
#     disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=target_names)
#     disp.plot(ax=ax, values_format="d", cmap=plt.cm.Blues, colorbar=False)
#     ax.set_title("Confusion Matrix")
#
#
# def plot_binary_roc_curve(ax, y_true_bin, y_prob, auc_value):
#     fpr, tpr, _ = roc_curve(y_true_bin, y_prob, pos_label=1)
#     ax.plot(fpr, tpr, color="blue", label=f"AUC = {auc_value:.4f}")
...
# 在绘图函数中修改颜色
def plot_loss_curve(ax, history_df: pd.DataFrame):
    ax.plot(history_df["epoch"], history_df["train_loss"], color="#c9171c", label="Train Loss")
    ax.plot(history_df["epoch"], history_df["val_loss"], color="#2877b8", label="Val Loss")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title("Loss Curve")
    ax.legend()
...
def plot_accuracy_curve(ax, history_df: pd.DataFrame):
    ax.plot(history_df["epoch"], history_df["train_acc"], color="#c9171c", label="Train Accuracy")
    ax.plot(history_df["epoch"], history_df["val_acc"], color="#2877b8", label="Val Accuracy")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Accuracy")
    ax.set_title("Accuracy Curve")
    ax.legend()
...
#     cm = confusion_matrix(y_true, y_pred, labels=np.arange(len(target_names)))
#     disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=target_names)
#     disp.plot(ax=ax, values_format="d", cmap=plt.cm.Blues, colorbar=False)  # 保留 cmap 默认，不改
#     ax.set_title("Confusion Matrix")
def plot_binary_confusion_matrix(ax, y_true, y_pred, target_names):
    cm = confusion_matrix(y_true, y_pred, labels=np.arange(len(target_names)))
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=target_names)
...
def plot_binary_roc_curve(ax, y_true_bin, y_prob, auc_value):
    fpr, tpr, _ = roc_curve(y_true_bin, y_prob, pos_label=1)
    ax.plot(fpr, tpr, color="#2877b8", label=f"AUC = {auc_value:.4f}")
...
def save_multiclass_outputs(
    ...
    multi8_save_dir = os.path.join(save_dir, "multiclass_8class")
    ...
    print(f"8-class prediction file saved to: {pred_csv_path}")

    true_col = detect_multiclass_true_label_col(real_data, g)
    if true_col is None:
        print("Warning: 未识别到 8 分类真实标签列，只保存预测结果，不计算 8 分类指标。")
        return None

    print(f"Detected 8-class true label column: {true_col}")
    ...
    with open(os.path.join(multi8_save_dir, "classification_report_8class.txt"), "w", encoding="utf-8") as f:
        f.write(report_8)

    cm_8 = confusion_matrix(y_true_8, y_pred_8, labels=np.arange(1, g + 1))
    disp_8 = ConfusionMatrixDisplay(
        confusion_matrix=cm_8,
        display_labels=[str(i) for i in range(1, g + 1)]
    )
    # fig, ax = plt.subplots(figsize=(8, 7))
    # disp_8.plot(ax=ax, values_format="d", cmap=plt.cm.Blues, colorbar=False)
    # ax.set_title("8-Class Confusion Matrix")
    # plt.tight_layout()
    # save_eps_and_png(fig, os.path.join(multi8_save_dir, "confusion_matrix_8class.eps"))
    # plt.close()
    fig, ax = plt.subplots(figsize=(8, 7))
    disp_8.plot(ax=ax, values_format="d", cmap=plt.cm.Blues, colorbar=False)
...
def main():
    ...
    print("\n===== Step B: Train / Val split inside 90% =====")
    ...
    print("\n===== Step D: Threshold selected on validation set =====")
    ...
    print(f"AUC: {final_metrics['auc'] if final_metrics['auc'] is None else format(final_metrics['auc'], '.4f')}")
    print(f"Negative Precision ({final_metrics['negative_class_label']}): {final_metrics['negative_precision']:.4f}")
    print(f"Negative Recall    ({final_metrics['negative_class_label']}): {final_metrics['negative_recall']:.4f}")
    print(f"Positive Precision ({final_metrics['positive_class_label']}): {final_metrics['positive_precision']:.4f}")
    print(f"Positive Recall    ({final_metrics['positive_class_label']}): {final_metrics['positive_recall']:.4f}")
    print(f"Positive F1        ({final_metrics['positive_class_label']}): {final_metrics['positive_f1']:.4f}")
    ...
    history_df = pd.DataFrame({
        "epoch": np.arange(1, len(result["history"]["train_loss"]) + 1),
    ...
    plot_loss_curve(ax, history_df)
    plt.tight_layout()
    save_eps_and_png(fig, os.path.join(SAVE_DIR, "loss_curve.eps"))
    plt.close()

    # 2) 单独保存 accuracy 图
    fig, ax = plt.subplots(figsize=(7, 5))
    plot_accuracy_curve(ax, history_df)
    plt.tight_layout()
    save_eps_and_png(fig, os.path.join(SAVE_DIR, "accuracy_curve.eps"))
    plt.close()

    # 3) 单独保存混淆矩阵
    fig, ax = plt.subplots(figsize=(6, 5))
    plot_binary_confusion_matrix(ax, test_result["test_true"], test_result["test_pred"], target_names)
    plt.tight_layout()
    save_eps_and_png(fig, os.path.join(SAVE_DIR, "confusion_matrix_holdout_test.eps"))
    plt.close()
    ...
    if len(np.unique(test_result["test_true"])) >= 2:
        ...
        fig, ax = plt.subplots(figsize=(6, 5))
        plot_binary_roc_curve(ax, y_true_bin, test_result["test_prob"], test_result["auc"])
        plt.tight_layout()
        save_eps_and_png(fig, os.path.join(SAVE_DIR, "roc_curve_holdout_test.eps"))
        plt.close()


    # =======================================================
    # G_Results. 保存模型、预处理器、编码器、二分类预测结果
    # =======================================================
    torch.save(result["best_state"], os.path.join(SAVE_DIR, "best_model_state_dict.pth"))
    joblib.dump(best_scaler, os.path.join(SAVE_DIR, "scaler.pkl"))
    ...
    pred_df.to_csv(os.path.join(SAVE_DIR, "holdout_test_predictions.csv"), index=False)
    ...
    if os.path.exists(MULTI8_DATA_PATH):
        ...
        print(f"\nWarning: 未找到 8 分类数据文件 {MULTI8_DATA_PATH}，跳过 8 分类预测与评估。")

    # =======================================================
    # I. 汇总保存
    # =======================================================
    summary = {
        "binary_final_metrics": final_metrics,
        "multiclass_8class_metrics": multiclass_metrics,
    }
    with open(os.path.join(SAVE_DIR, "all_summary_metrics.json"), "w", encoding="utf-8") as f:
